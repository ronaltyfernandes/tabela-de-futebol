...
function calcularPontos(team:teamsType, match:matchesType, valores:lboardType):
...
function leaderBoardesValues(team: teamsType, matchesList: matchesType[]) {
  ...
  matchesList.forEach((match) => {
    const validGame = calcularPontos(team, match, valores);
...
export default class LeaderBoardsModel implements ILeaderBoardModel {
  ...
  async findAll(): Promise<lboardType[]> {
    ...
    const values = teamsList.map((team:teamsType) => {
      const returnValues = leaderBoardesValues(team, matchesList);
      return returnValues;
    });
